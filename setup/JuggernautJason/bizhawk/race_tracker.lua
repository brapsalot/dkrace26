...
local STREAMER_NAME = "JuggernautJason"
...
-- Track the last KNOWN level info so checkpoint/bonus room IDs
-- don't show "Unknown" (they reuse the parent level's info)
local last_known_name = nil
local last_known_world = -1
local last_known_level = -1
local last_known_progress = -1
...
local function read_progress()
  ...
  local levelName, worldIndex, levelIndex, progressIndex

  if info then
    -- Known area ID — update tracking
    levelName = info.name
    worldIndex = info.worldIndex
    levelIndex = info.levelIndex
    progressIndex = info.progressIndex
    last_known_name = levelName
    last_known_world = worldIndex
    last_known_level = levelIndex
    last_known_progress = progressIndex
  elseif last_known_name then
    -- Unknown area ID but we have a previous level — use it
    -- (checkpoint restart, bonus room, mid-level transition)
    levelName = last_known_name
    worldIndex = last_known_world
    levelIndex = last_known_level
    progressIndex = last_known_progress
  else
    -- No previous level known yet
    levelName = string.format("Unknown (0x%02X)", levelId)
    worldIndex = -1
    levelIndex = -1
    progressIndex = -1
  end
